...
class Utils():
    ...
    def measure_this(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Try to extract username from request args or json
            username = None
            if len(args) > 0 and hasattr(args[0], 'args') and args[0].args.get('username'):
                username = args[0].args.get('username')
            elif len(args) > 0 and hasattr(args[0], 'json') and args[0].json and args[0].json.get('username'):
                username = args[0].json.get('username')
            ...
            if username:
                self.logger.info(f"EXECUTION TIME FOR {func.__name__} (user: {username}): {execution_time:.4f} SECONDS")
            elif class_name:
